    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                genver_multi_objective_enable = bool(meta_info.get("genver_multi_objective_enable", False))
                genver_weights = meta_info.get("genver_multi_objective_weights", {})
                genver_weight_answer = float(genver_weights.get("answer", 1.0))
                genver_weight_loc = float(genver_weights.get("loc", 1.0))
                genver_weight_loc_verifier = float(genver_weights.get("loc_verifier", 1.0))
                genver_weight_answer_logic_verifier = float(genver_weights.get("answer_logic_verifier", 1.0))

                if (
                    genver_multi_objective_enable
                    and "genver_adv_answer" in data
                    and "genver_mask_answer" in data
                    and "genver_adv_loc" in data
                    and "genver_mask_loc" in data
                    and "genver_adv_loc_verifier" in data
                    and "genver_mask_loc_verifier" in data
                    and "genver_adv_answer_logic_verifier" in data
                    and "genver_mask_answer_logic_verifier" in data
                ):
                    ...
                    response_mask_for_regularizers = (
                        (data["genver_mask_answer"] > 0)
                        | (data["genver_mask_loc"] > 0)
                        | (data["genver_mask_loc_verifier"] > 0)
                        | (data["genver_mask_answer_logic_verifier"] > 0)
                    ).to(bool)
                else:
